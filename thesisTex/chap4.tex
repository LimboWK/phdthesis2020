\chapter{$B^0$ reconstruction study}
...
\begin{figure}[H]
	\begin{minipage}[b]{0.5\linewidth}
		\centering 
		\includegraphics[height=5cm]{figures/best_ncands_noBCS}
		
	\end{minipage}
	\begin{minipage}[b]{0.5\linewidth}
		\centering 
		\includegraphics[height=5cm]{figures/best_treeFitChi2_noBCS}
		
	\end{minipage}
	\begin{minipage}[b]{0.5\linewidth}
		\centering 
		\includegraphics[height=5cm]{figures/best_ncands_sig}
		
	\end{minipage}
	\begin{minipage}[b]{0.5\linewidth}
		\centering 
		\includegraphics[height=5cm]{figures/hist_sig_MC_Mbc_dE}
		
	\end{minipage}
	
	\caption{Top left is the $B^0$ candidates per event in data and \textit{generic MC} before the BCS. Top Right is the $\chi^2$ for data and \textit{generic MC} before BCS. Bottom left is the number of $B^0$ candidates per event from\textit{signal MC}. Bottom right is the 2D $M_{bc}$ and $\Delta E$ distribution from \textit{signal MC}.}
	\label{fig:b0dist}
\end{figure}
% move to after CS
...
The number of these background in signal reconstruction could be further reduced by implementing veto on invariant mass of 2$K_S^0$. However, such veto should be carefully validated with data. The distribution of invariant mass of \textit{X} should agree well in MC and data, which is hard to check in the low luminosity. The distribution of 2$K_S^0$ invariant mass in \textit{generic MC} and data are shown in the   C.
...
\end{equation}
 ...
 \begin{figure}[htbp]
 	\begin{minipage}[b]{0.5\linewidth}
 		\centering 
 		\includegraphics[height=6cm]{mbc-hist}
 		%\label{fig:side:a}
 	\end{minipage}
 	\begin{minipage}[b]{0.5\linewidth}
 		\centering 
 		\includegraphics[height=6cm]{dE-hist}
 		%\label{fig:side:b}
 	\end{minipage}
...
\end{minipage}
\caption{Invariant mass before(left) and after $B^0$ vertext fit(right) from \textit{generic MC} and experiment data.}
\label{fig:b0ksmass}
\end{figure}
